...
public class MailingCommandTests : TestBase
{
    ...
    public async Task ShouldReturnFailedResult_Because_CityIsEmpty()
    {
        ...
        var message = GenerateMessageWithPayload(DefaultUser.ConsumerId, DefaultUser.ConsumerId, command.Trigger, "weather");
    ...
    public async Task ShouldReturnFailedResult_Because_UserGroupIsZero()
    {
        ...
        var command = new MailingCommand(GetDbContext());
        var message = GenerateMessageWithPayload(DefaultUser.ConsumerId, DefaultUser.ConsumerId, command.Trigger, "schedule");
