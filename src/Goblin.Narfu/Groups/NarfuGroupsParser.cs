...
internal sealed partial class NarfuGroupsParser(
    ...
    ILogger<NarfuGroupsParser> logger) : INarfuGroupsParser
{
    ...
    private static IReadOnlyCollection<Group> ParseGroups(string html)
    {
        ...
        var groups = new List<Group>();
        foreach(var link in document.DocumentNode.SelectNodes("//a[@href][.//span[contains(concat(' ', normalize-space(@class), ' '), ' number ')]]") ?? Enumerable.Empty<HtmlNode>())
        {
