...
{
    public class TeachersSchedule : TestBase
    {
        [Fact]
        public async void GetSchedule_CorrectId_Lessons()
        {
            using(var httpTest = new HttpTest())
            {
                ...
                var result = await GetService().Teachers.GetSchedule(22585);
        ...
        public async void GetScheduleAsString_CorrectId_String(int id)
        {
            using(var httpTest = new HttpTest())
            {
                ...
                var result = await GetService().Teachers.GetScheduleAsString(id);
        ...
        {
            var result = GetService().Teachers.FindByName(name);
        ...
        {
            var result = GetService().Teachers.FindById(id);
